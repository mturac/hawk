...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    const db = await getDb();
    const existing = queryOne(db, 'SELECT id FROM repos WHERE full_name = ?', [full_name]);
    if (existing) {
    ...
    const result = runSql(db, `
      ...
      VALUES (?, ?, ?, ?, ?)
    `, [github_id || 0, owner, name, full_name, webhookSecret]);
...
router.put('/:id', async (req: Request, res: Response) => {
  try {
    ...
    const repo = queryOne(db, 'SELECT * FROM repos WHERE id = ?', [req.params.id]);
    if (!repo) {
      return res.status(404).json({ error: 'Repository not found' });
    }

    if (enabled !== undefined) {
      runSql(db, 'UPDATE repos SET enabled = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [enabled ? 1 : 0, req.params.id]);
    }

    if (config) {
      runSql(db, 'UPDATE repos SET config_json = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [JSON.stringify(config), req.params.id]);
    }

    const updated = queryOne(db, 'SELECT * FROM repos WHERE id = ?', [req.params.id]);
...
router.delete('/:id', async (req: Request, res: Response) => {
  try {
    const db = await getDb();
    const result = runSql(db, 'DELETE FROM repos WHERE id = ?', [req.params.id]);
